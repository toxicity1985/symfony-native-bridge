...
class TrayManagerSyncTest extends TestCase
{
    ...
    public function testSyncFromRuntimeDoesNotDuplicateAlreadyTrackedTrays(): void
    {
        ...
        // 'tray_1' is still accessible via its original 'main' label
        $this->driver->expects($this->once())->method('destroyTray')->with('tray_1');
        $this->manager->destroy('main');

        // 'tray_orphan' is also accessible via its ID as label
        $this->driver->expects($this->once())->method('destroyTray')->with('tray_orphan');
